-- Test script for fixed visitor analytics functions

-- Clean up any existing test data
DELETE FROM public.visitor_analytics WHERE session_id = 'test_session_fixed_001';

-- Test 1: Create a visitor session
SELECT * FROM public.get_or_create_visitor_session(
    'test_session_fixed_001',
...
SELECT public.increment_cart_additions('test_session_fixed_001');

-- Test 3: Increment checkout initiations
SELECT public.increment_checkout_initiations('test_session_fixed_001');

-- Test 4: Increment completed orders
SELECT public.increment_completed_orders('test_session_fixed_001');
...
WHERE session_id = 'test_session_fixed_001';
...
-- Test 7: Test with NULL session ID (should fail)
