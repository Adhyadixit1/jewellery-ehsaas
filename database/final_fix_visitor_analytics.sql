-- Final fix for visitor analytics functions to resolve 400 Bad Request error
-- This fix specifically addresses the "column reference 'session_id' is ambiguous" error
...
-- This version fixes the ambiguous column reference by using table aliases
...
BEGIN
    ...
    -- Fix the ambiguous column reference by using table alias
    SELECT va.id INTO v_visitor_id
    FROM public.visitor_analytics va
    WHERE va.session_id = p_session_id
...
BEGIN
    ...
    -- Fix the ambiguous column reference by using table alias
    UPDATE public.visitor_analytics va
    SET added_to_cart_count = COALESCE(va.added_to_cart_count, 0) + 1,
        updated_at = NOW()
    WHERE va.session_id = p_session_id;
...
BEGIN
    ...
    -- Fix the ambiguous column reference by using table alias
    UPDATE public.visitor_analytics va
    SET initiated_checkout_count = COALESCE(va.initiated_checkout_count, 0) + 1,
        updated_at = NOW()
    WHERE va.session_id = p_session_id;
...
BEGIN
    ...
    -- Fix the ambiguous column reference by using table alias
    UPDATE public.visitor_analytics va
    SET completed_orders_count = COALESCE(va.completed_orders_count, 0) + 1,
        updated_at = NOW()
    WHERE va.session_id = p_session_id;
